...
struct _st_epolldata *st_epoll_data[MAX_ST_VP];
extern __thread int self_index;
#define _st_epoll_data (st_epoll_data[self_index])
...
ST_HIDDEN void _st_select_dispatch(void)
{
	...
	if (_ST_SLEEPQ(self_index) == NULL) {
		tvp = NULL;
	} else {
		min_timeout =
				(_ST_SLEEPQ(self_index)->due <= _ST_LAST_CLOCK) ?
						0 : (_ST_SLEEPQ(self_index)->due - _ST_LAST_CLOCK);
...
ST_EVENTSYS_SELECT, _st_select_init, _st_select_dispatch,
		_st_select_pollset_add, _st_select_pollset_del, _st_select_fd_new,
		_st_select_fd_close, _st_select_fd_getlimit };
...
ST_HIDDEN int _st_poll_init(void)
{
	...
	_ST_POLLFDS = (struct pollfd *) malloc(
			ST_MIN_POLLFDS_SIZE * sizeof(struct pollfd));
	if (!_ST_POLLFDS) {
		free(_st_poll_data);
		_st_poll_data = NULL;
		return -1;
...
ST_HIDDEN void _st_poll_dispatch(void)
{
	...
	if (_ST_POLL_OSFD_CNT > _ST_POLLFDS_SIZE) {
		free(_ST_POLLFDS);
		_ST_POLLFDS = (struct pollfd *) malloc(
				(_ST_POLL_OSFD_CNT + 10) * sizeof(struct pollfd));
...
ST_EVENTSYS_POLL, _st_poll_init, _st_poll_dispatch, _st_poll_pollset_add,
		_st_poll_pollset_del, _st_poll_fd_new, _st_poll_fd_close,
...
ST_HIDDEN int _st_epoll_init(void)
{
	...
	_st_epoll_data = (struct _st_epolldata *) calloc(1,
			sizeof(*_st_epoll_data));
	if (!_st_epoll_data)
		return -1;

	fdlim = st_getfdlimit();
	_st_epoll_data->fd_hint =
...
ST_EVENTSYS_ALT, _st_epoll_init, _st_epoll_dispatch, _st_epoll_pollset_add,
		_st_epoll_pollset_del, _st_epoll_fd_new, _st_epoll_fd_close,
