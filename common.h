...
static inline void st_pthread_spin_init(st_pthread_spinlock* lock)
{
	pthread_spin_init(&lock->lock, 0);
	lock->locked_thread = 0;
	lock->ref = 0;
}

static inline void st_pthread_spin_lock(st_pthread_spinlock* lock)
{
	if (lock->locked_thread == pthread_self()) {
		lock->ref++;
		return;
	}
	pthread_spin_lock(&lock->lock);
	lock->ref++;
	lock->locked_thread = pthread_self();
}

static inline void st_pthread_spin_unlock(st_pthread_spinlock* lock)
{
	if (lock->locked_thread != pthread_self()) {
		return;
	}
...
{
	_st_thread_t *main_thread;
	int eventfd; /* for job passing */
	_st_thread_t *event_thread;
	...
	int run_q_size;
	st_pthread_spinlock run_q_lock;
	...
	st_pthread_spinlock _sleep_q_lock;
...
#ifdef DEBUG
#define ST_DEBUG_PRINTF(msg, ...) log_out_msg(__func__, msg, ##__VA_ARGS__)
#else
 ...
 */
#define _ST_ADD_SWITCHQ(_thr) ST_APPEND_LINK(&(_thr)->links, &_ST_SWITCHQ(self_index))
...
#define _ST_IS_EMPTY        (&_ST_RUNQ(self_index) == _ST_RUNQ(self_index).next)
...
#define _ST_FL_PTREAD_SPIN  0x20
